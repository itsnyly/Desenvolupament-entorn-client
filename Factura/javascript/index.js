...
class Factura{
    ...
    mostrarFactura(){
        ...
        this.articles.forEach(element => {
            ...
            fila.innerHTML="<tr><td>"+element["article"].codiArticle+"</td><td>"+element["article"].nomArticle+"</td><td><input class='inputArticle' type=number name="+element["article"].codiArticle+" value ="+element["quantitatArticle"]+"></input></td><td id= preuArticle>"+element["article"].preuArticle+"</td><td id= preuTotalArticle>"+element["quantitatArticle"] * element["article"].preuArticle+"</td><td><button class='botoSumar' id= botoSumar>+</button></td><td><button class='botoRestar' id= botoRestar>-</button></td><td><input class='inputDescompte' type=number name="+element["article"].codiArticle+" value ="+element["quantitatArticle"]+"></input></td></tr>";
        ...
        document.querySelectorAll(".inputDescompte").forEach(element => element.addEventListener("change",this.canviarPreu));
        //document.getElementById("botoSumar").addEventListener("click", this.canviarQuantitatBotoSumar);
        //document.getElementById("botoRestar").addEventListener("click", this.canviarQuantitatBotoRestar);
    ...
    canviarQuantitatBotoSumar(){
        console.log("entra");
        console.log(factura.articles["quantitatArticle"]);
        let index = factura.articles.findIndex(element => element["article"].codiArticle == element["quantitatArticle"].getAttribute("name"));
        console.log(index);
        let trobat = factura.articles[index];
        trobat["quantitatArticle"] = trobat["quantitatArticle"] +1;
        if(trobat["quantitatArticle"] <=0){
            factura.articles.splice(index,1);
        }
        factura.mostrarFactura();
        factura.calcularTotal();
    }
    canviarQuantitatBotoRestar(){
        let index = factura.articles.findIndex(element => element["article"].codiArticle == this.getAttribute("name"));
        let trobat = factura.articles[index];
        trobat["quantitatArticle"] = trobat["quantitatArticle"] -1;
        if(trobat["quantitatArticle"] <=0){
