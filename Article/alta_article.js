...
function afegirValorPoblacio(valor){
    let desplegable = document.getElementsByName("Poblacio");
    let option = new Option(valor,valor);
    desplegable[0].append(option);
   
   }
   
   function carregarDesplegablePoblacio(){
       let arrayDesplegable = ["Valencia", "Girona", "Barcelona", "Madrid", "Sevilla", "Tarragona","Alicante","Lleida"];
       let arrayOrdenada = arrayDesplegable.sort();
       for (let i = 0; i < arrayDesplegable.length; i++ ){
           afegirValorPoblacio(arrayOrdenada[i]);
       }
   }
...
function inicialsPoblacio(){
    let valorPoblacio = document.getElementsByTagName("select")[1].value;
    let inicials =  valorPoblacio.slice(0,5);
    return inicials;
}

function Edifici(){
    let textEdifici = document.getElementById("Edifici");
    textEdifici.addEventListener("change", comprovarEdifici);
}

function comprovarEdifici(){
    let inicials = inicialsPoblacio();
    let arrayInicials = inicials.split("");
    let valorEdifici = document.getElementById("Edifici").value;
    let regEx = new RegExp("^["+arrayInicials[0].toUpperCase()+arrayInicials[0].toLowerCase()+"]["+arrayInicials[1].toUpperCase()+arrayInicials[1].toLowerCase()+"]["+arrayInicials[2].toUpperCase()+arrayInicials[2].toLowerCase()+"]["+arrayInicials[3].toUpperCase()+arrayInicials[3].toLowerCase()+"]["+arrayInicials[4].toUpperCase()+arrayInicials[4].toLowerCase()+"]"+"\\+[0-9]{3}\.[a-z]{2}$");
    let resultatEdifici = regEx.test(valorEdifici);
    canviarImg(4,resultatEdifici);
    return resultatEdifici;
}
